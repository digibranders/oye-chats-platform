"""Single production baseline.

Replaces the 61 incremental revisions that ran from the first schema to
``9c4d31e07b52``. Every one of them had already been applied everywhere it was
ever going to be applied, so their only remaining job was to be replayed, in
order, against each new database — 61 transactions to reach a schema this one
reaches in a single statement.

The SQL below is `pg_dump --schema-only` of production at the squash, not a
hand-written or autogenerated approximation. It was verified by applying it to
an empty database on the production host and diffing the result against
production itself: identical but for pg_dump's per-run `\\restrict` token and
one CHECK constraint that Postgres re-renders with a different array cast.
Autogenerate was not used, because it does not reliably reproduce partial
indexes, server defaults, or the pgvector column types this schema depends on.

The `COMMENT ON EXTENSION` lines pg_dump emits alongside `CREATE EXTENSION` are
dropped: they require ownership of the extension, and migrations run as the
application role, which does not own `citext` or `vector`. Installing an
extension needs superuser regardless, so both are provisioned with the database
...
revision: str = "a0000000baseline"
down_revision: str | None = None
branch_labels: None = None
depends_on: None = None


# `exec_driver_sql`, not `op.execute`: this is one multi-statement script and
# SQLAlchemy would otherwise read every `:` inside a CHECK constraint or a
# default as a bind parameter.
SCHEMA_SQL = r"""
...
CREATE TABLE public.addon_plan_cache (
    id integer NOT NULL,
    addon_kind character varying(32) NOT NULL,
    currency character varying(3) DEFAULT 'INR'::character varying NOT NULL,
    amount_minor integer NOT NULL,
    razorpay_plan_id character varying NOT NULL,
    created_at timestamp with time zone DEFAULT now(),
    CONSTRAINT chk_addon_plan_amount_positive CHECK ((amount_minor > 0))
);
CREATE SEQUENCE public.addon_plan_cache_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1;
ALTER SEQUENCE public.addon_plan_cache_id_seq OWNED BY public.addon_plan_cache.id;
...
CREATE TABLE public.billing_funnel_events (
    id integer NOT NULL,
    client_id integer NOT NULL,
    event character varying(24) NOT NULL,
    surface character varying(12) NOT NULL,
    meta jsonb,
    created_at timestamp with time zone DEFAULT now() NOT NULL
);
CREATE SEQUENCE public.billing_funnel_events_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1;
ALTER SEQUENCE public.billing_funnel_events_id_seq OWNED BY public.billing_funnel_events.id;
CREATE TABLE public.bot_domain_installs (
    id integer NOT NULL,
    bot_id integer NOT NULL,
    hostname character varying(253) NOT NULL,
    observed_first_at timestamp with time zone,
    observed_last_at timestamp with time zone,
    probe_status character varying(16),
    probe_checked_at timestamp with time zone,
    probe_bot_key character varying(64),
    probe_detail character varying(200),
    created_at timestamp with time zone DEFAULT now(),
    CONSTRAINT chk_bot_domain_probe_status CHECK (((probe_status IS NULL) OR ((probe_status)::text = ANY ((ARRAY['installed'::character varying, 'foreign'::character varying, 'missing'::character varying, 'unreachable'::character varying])::text[]))))
);
CREATE SEQUENCE public.bot_domain_installs_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1;
ALTER SEQUENCE public.bot_domain_installs_id_seq OWNED BY public.bot_domain_installs.id;
CREATE TABLE public.bot_growth_events (
    id integer NOT NULL,
    bot_id integer NOT NULL,
    event_type character varying NOT NULL,
    created_at timestamp with time zone DEFAULT now(),
    CONSTRAINT ck_bot_growth_events_event_type CHECK (((event_type)::text = ANY (ARRAY[('demo_share_clicked'::character varying)::text, ('demo_link_opened'::character varying)::text])))
);
...
CREATE TABLE public.bots (
    ...
    launcher_logo text,
    primary_color character varying DEFAULT '#a21caf'::character varying,
    ...
    branding_url character varying DEFAULT 'https://www.oyechats.com'::character varying NOT NULL,
    services jsonb,
    ...
    last_crawl_status character varying,
    crawl_completed_at timestamp with time zone,
    indexed_chunk_count integer DEFAULT 0 NOT NULL,
    session_share_domain character varying,
    followup_sending_paused boolean DEFAULT false,
    email_verification_enabled boolean DEFAULT false NOT NULL,
    company_lookup_enabled boolean DEFAULT false NOT NULL,
    bot_logo_source character varying,
    answer_links jsonb,
    language_config jsonb DEFAULT '{"enabled": false, "auto_detect": true, "default_locale": "en-IN", "supported_locales": ["en-IN"], "operator_translation_enabled": false, "allow_visitor_language_switch": false}'::jsonb NOT NULL,
    qualification_flow jsonb,
    quotation_catalog jsonb,
    widget_last_seen_at timestamp with time zone,
    widget_last_origin character varying,
    demo_screenshot_url character varying,
    demo_screenshot_captured_at timestamp with time zone,
    demo_screenshot_source_url character varying,
    demo_screenshot_status character varying,
    dev_invite_email character varying,
    dev_invite_sent_at timestamp with time zone
);
...
CREATE TABLE public.chat_messages (
    ...
    created_at timestamp with time zone DEFAULT now(),
    is_unanswered boolean DEFAULT false NOT NULL,
    source_language character varying(16),
    translations jsonb
);
...
CREATE TABLE public.clients (
    ...
    signup_promo_code character varying,
    geo_mismatch_at timestamp with time zone,
    geo_mismatch_detail character varying(500),
    pending_checkout_subscription_id character varying,
    pending_checkout_plan_id integer,
    pending_checkout_cycle character varying(8),
    pending_checkout_country character varying(2),
    pending_checkout_at timestamp with time zone,
    kb_characters_used bigint DEFAULT '0'::bigint NOT NULL,
    notification_preferences jsonb,
    pending_checkout_bot_id integer
);
...
CREATE TABLE public.company_profile (
    ...
    resolution_failed boolean DEFAULT false NOT NULL,
    ...
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
    CONSTRAINT chk_company_profile_domain_length CHECK (((length((domain)::text) >= 1) AND (length((domain)::text) <= 253)))
);
...
CREATE TABLE public.invoices (
    ...
    inr_amount_minor bigint,
    inr_taxable_value_minor bigint,
    inr_total_tax_minor bigint,
...
CREATE TABLE public.lead_info (
    ...
    followup_sent_by_operator_id integer,
    company_name character varying,
    company_description text,
...
CREATE TABLE public.operator_expo_push_tokens (
    id integer NOT NULL,
    operator_id integer,
    client_id integer,
    ...
    CONSTRAINT chk_expo_push_token_owner_xor CHECK (((((operator_id IS NULL))::integer + ((client_id IS NULL))::integer) = 1))
);
CREATE SEQUENCE public.operator_expo_push_tokens_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1;
ALTER SEQUENCE public.operator_expo_push_tokens_id_seq OWNED BY public.operator_expo_push_tokens.id;
...
CREATE TABLE public.operators (
    ...
    preferred_locale character varying(32),
...
CREATE TABLE public.plans (
    ...
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
    razorpay_plan_id_monthly_usd character varying,
    razorpay_plan_id_annual_usd character varying,
...
CREATE TABLE public.promotions (
    ...
    free_cycles integer DEFAULT 3 NOT NULL,
    eligible_plan_ids jsonb,
    max_redemptions integer,
    redeemed_count integer DEFAULT 0 NOT NULL,
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
    CONSTRAINT ck_promotions_free_cycles_positive CHECK ((free_cycles > 0)),
    CONSTRAINT ck_promotions_max_redemptions_positive CHECK (((max_redemptions IS NULL) OR (max_redemptions > 0))),
    CONSTRAINT ck_promotions_window_order CHECK ((ends_at > starts_at))
);
...
CREATE TABLE public.subscriptions (
    ...
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
    dunning_emails_sent jsonb DEFAULT '{}'::jsonb NOT NULL,
    gateway_cancel_executed_at timestamp with time zone,
    promotion_id integer,
    promo_free_until timestamp with time zone,
    promo_reminder_sent jsonb DEFAULT '{}'::jsonb NOT NULL,
    branding_addon_subscription_id character varying,
    branding_addon_active boolean DEFAULT false NOT NULL,
    branding_addon_pending boolean DEFAULT false NOT NULL
);
...
ALTER TABLE ONLY public.addon_plan_cache ALTER COLUMN id SET DEFAULT nextval('public.addon_plan_cache_id_seq'::regclass);
...
ALTER TABLE ONLY public.billing_funnel_events ALTER COLUMN id SET DEFAULT nextval('public.billing_funnel_events_id_seq'::regclass);
ALTER TABLE ONLY public.bot_domain_installs ALTER COLUMN id SET DEFAULT nextval('public.bot_domain_installs_id_seq'::regclass);
...
ALTER TABLE ONLY public.operator_expo_push_tokens ALTER COLUMN id SET DEFAULT nextval('public.operator_expo_push_tokens_id_seq'::regclass);
...
ALTER TABLE ONLY public.addon_plan_cache
    ADD CONSTRAINT addon_plan_cache_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.billing_funnel_events
    ADD CONSTRAINT billing_funnel_events_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.bot_domain_installs
    ADD CONSTRAINT bot_domain_installs_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.company_profile
    ADD CONSTRAINT company_profile_pkey PRIMARY KEY (domain);
...
ALTER TABLE ONLY public.operator_expo_push_tokens
    ADD CONSTRAINT operator_expo_push_tokens_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.addon_plan_cache
    ADD CONSTRAINT uq_addon_plan_amount UNIQUE (addon_kind, currency, amount_minor);
ALTER TABLE ONLY public.bot_domain_installs
    ADD CONSTRAINT uq_bot_domain_install UNIQUE (bot_id, hostname);
ALTER TABLE ONLY public.discounted_plan_cache
    ADD CONSTRAINT uq_discounted_plan UNIQUE (base_plan_id, billing_cycle, discount_bps, currency);
ALTER TABLE ONLY public.email_suppressions
    ADD CONSTRAINT uq_email_suppressions_bot_email UNIQUE (bot_id, email);
ALTER TABLE ONLY public.events
    ADD CONSTRAINT uq_events_bot_source_title UNIQUE (bot_id, source_url, title);
ALTER TABLE ONLY public.operator_expo_push_tokens
    ADD CONSTRAINT uq_operator_expo_push_token UNIQUE (token);
ALTER TABLE ONLY public.operator_push_subscriptions
    ADD CONSTRAINT uq_operator_push_endpoint UNIQUE (endpoint);
ALTER TABLE ONLY public.referral_conversions
    ADD CONSTRAINT uq_referral_conversions_client UNIQUE (client_id);
...
CREATE INDEX ix_billing_funnel_events_client_id ON public.billing_funnel_events USING btree (client_id);
CREATE INDEX ix_billing_funnel_events_created_at ON public.billing_funnel_events USING btree (created_at);
CREATE INDEX ix_bot_domain_installs_bot_id ON public.bot_domain_installs USING btree (bot_id);
...
CREATE INDEX ix_chat_messages_is_unanswered ON public.chat_messages USING btree (is_unanswered);
CREATE INDEX ix_chat_messages_session_id_created ON public.chat_messages USING btree (session_id, created_at DESC);
CREATE INDEX ix_chat_sessions_bot_id_created ON public.chat_sessions USING btree (bot_id, created_at DESC);
...
CREATE UNIQUE INDEX ix_clients_razorpay_customer_id ON public.clients USING btree (razorpay_customer_id);
CREATE INDEX ix_company_profile_refresh_due ON public.company_profile USING btree (refresh_after) WHERE (NOT resolution_failed);
CREATE INDEX ix_company_profile_retry_due ON public.company_profile USING btree (retry_after) WHERE resolution_failed;
CREATE UNIQUE INDEX ix_coupons_code ON public.coupons USING btree (code);
CREATE INDEX ix_credit_ledger_attributed_bot_id ON public.credit_ledger USING btree (attributed_bot_id);
...
CREATE INDEX ix_documents_bot_id_is_active ON public.documents USING btree (bot_id, is_active);
...
CREATE INDEX ix_documents_is_active ON public.documents USING btree (is_active);
CREATE INDEX ix_documents_search_vector ON public.documents USING gin (search_vector);
CREATE INDEX ix_email_suppressions_bot_id ON public.email_suppressions USING btree (bot_id);
...
CREATE INDEX ix_operator_expo_push_tokens_client_id ON public.operator_expo_push_tokens USING btree (client_id);
CREATE INDEX ix_operator_expo_push_tokens_operator_id ON public.operator_expo_push_tokens USING btree (operator_id);
CREATE INDEX ix_operator_invites_bot_id ON public.operator_invites USING btree (bot_id);
...
CREATE INDEX ix_reconciliation_runs_ran_at ON public.reconciliation_runs USING btree (ran_at);
...
CREATE UNIQUE INDEX ix_subscriptions_branding_addon_subscription_id ON public.subscriptions USING btree (branding_addon_subscription_id) WHERE (branding_addon_subscription_id IS NOT NULL);
CREATE UNIQUE INDEX ix_subscriptions_client_bot_active ON public.subscriptions USING btree (client_id, bot_id) WHERE ((bot_id IS NOT NULL) AND ((status)::text = ANY (ARRAY[('active'::character varying)::text, ('trialing'::character varying)::text, ('past_due'::character varying)::text])));
CREATE INDEX ix_subscriptions_client_id ON public.subscriptions USING btree (client_id);
CREATE UNIQUE INDEX ix_subscriptions_client_legacy_active ON public.subscriptions USING btree (client_id) WHERE ((bot_id IS NULL) AND ((status)::text = ANY (ARRAY[('active'::character varying)::text, ('trialing'::character varying)::text, ('past_due'::character varying)::text])));
...
CREATE UNIQUE INDEX uq_processed_webhooks_payload_digest ON public.processed_webhooks USING btree (payload_digest) WHERE (payload_digest IS NOT NULL);
...
ALTER TABLE ONLY public.billing_funnel_events
    ADD CONSTRAINT billing_funnel_events_client_id_fkey FOREIGN KEY (client_id) REFERENCES public.clients(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.bot_domain_installs
    ADD CONSTRAINT bot_domain_installs_bot_id_fkey FOREIGN KEY (bot_id) REFERENCES public.bots(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.bot_growth_events
...
ALTER TABLE ONLY public.credit_ledger
    ADD CONSTRAINT credit_ledger_attributed_bot_id_fkey FOREIGN KEY (attributed_bot_id) REFERENCES public.bots(id) ON DELETE SET NULL;
ALTER TABLE ONLY public.credit_ledger
...
ALTER TABLE ONLY public.email_suppressions
    ADD CONSTRAINT email_suppressions_bot_id_fkey FOREIGN KEY (bot_id) REFERENCES public.bots(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.events
...
ALTER TABLE ONLY public.subscriptions
    ADD CONSTRAINT fk_subscriptions_promotion_id FOREIGN KEY (promotion_id) REFERENCES public.promotions(id) ON DELETE SET NULL;
ALTER TABLE ONLY public.impersonation_tokens
...
ALTER TABLE ONLY public.lead_info
    ADD CONSTRAINT lead_info_followup_sent_by_operator_id_fkey FOREIGN KEY (followup_sent_by_operator_id) REFERENCES public.operators(id) ON DELETE SET NULL;
ALTER TABLE ONLY public.lead_info
...
ALTER TABLE ONLY public.operator_expo_push_tokens
    ADD CONSTRAINT operator_expo_push_tokens_client_id_fkey FOREIGN KEY (client_id) REFERENCES public.clients(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.operator_expo_push_tokens
    ADD CONSTRAINT operator_expo_push_tokens_operator_id_fkey FOREIGN KEY (operator_id) REFERENCES public.operators(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.operator_invites
...
ALTER TABLE ONLY public.operator_invites
    ADD CONSTRAINT operator_invites_bot_id_fkey FOREIGN KEY (bot_id) REFERENCES public.bots(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.operator_invites
...
DROP TABLE IF EXISTS public.addon_plan_cache CASCADE;
DROP TABLE IF EXISTS public.affiliate_invites CASCADE;
DROP TABLE IF EXISTS public.affiliates CASCADE;
DROP TABLE IF EXISTS public.audit_logs CASCADE;
DROP TABLE IF EXISTS public.bant_signals CASCADE;
DROP TABLE IF EXISTS public.billing_funnel_events CASCADE;
DROP TABLE IF EXISTS public.bot_domain_installs CASCADE;
DROP TABLE IF EXISTS public.bot_growth_events CASCADE;
DROP TABLE IF EXISTS public.bots CASCADE;
DROP TABLE IF EXISTS public.canned_responses CASCADE;
DROP TABLE IF EXISTS public.chat_audit_logs CASCADE;
...
DROP TABLE IF EXISTS public.discounted_plan_cache CASCADE;
DROP TABLE IF EXISTS public.documents CASCADE;
DROP TABLE IF EXISTS public.email_suppressions CASCADE;
DROP TABLE IF EXISTS public.events CASCADE;
DROP TABLE IF EXISTS public.failed_webhooks CASCADE;
DROP TABLE IF EXISTS public.impersonation_tokens CASCADE;
DROP TABLE IF EXISTS public.invoice_counters CASCADE;
DROP TABLE IF EXISTS public.invoices CASCADE;
DROP TABLE IF EXISTS public.lead_info CASCADE;
DROP TABLE IF EXISTS public.live_chat_queue CASCADE;
...
DROP TABLE IF EXISTS public.offline_messages CASCADE;
DROP TABLE IF EXISTS public.operator_expo_push_tokens CASCADE;
DROP TABLE IF EXISTS public.operator_invites CASCADE;
DROP TABLE IF EXISTS public.operator_push_subscriptions CASCADE;
DROP TABLE IF EXISTS public.operators CASCADE;
DROP TABLE IF EXISTS public.payment_methods CASCADE;
DROP TABLE IF EXISTS public.plans CASCADE;
DROP TABLE IF EXISTS public.platform_feedback CASCADE;
...
DROP TYPE IF EXISTS public.credit_reason CASCADE;
"""
...
def downgrade() -> None:
    # Drops what this revision created and nothing else, so the extensions and
    # `alembic_version` itself survive for alembic to record the downgrade.
