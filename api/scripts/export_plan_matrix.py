...
def build_matrix() -> dict[str, Any]:
    ...
    all_seed_plans = _load_seed_plans()
    plans = [p for p in all_seed_plans if p["is_public"]]
    published = {p["slug"] for p in plans}

    # The signup trial, carried separately from ``plans``. It is every new
    # account's starting state, so the docs must be able to state its length and
    # allowance, but it is not a tier anybody chooses and must never appear in a
    # comparison table. Exported as its own object for exactly that reason: the
    # prose that describes it stays generated rather than hand-written, which is
    # the drift this whole exporter exists to stop.
    trial_row = next((p for p in all_seed_plans if p["trial_days"] > 0), None)
    ...
    return {
        ...
        "trial": (
            {
                "name": trial_row["name"],
                "days": trial_row["trial_days"],
                "credits": trial_row["credits_per_month"],
                "max_crawl_pages": trial_row["limits"].get("max_crawl_pages"),
                "bots": trial_row["limits"].get("bots"),
                "operators": trial_row["limits"].get("operators"),
            }
