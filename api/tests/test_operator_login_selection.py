"""Tests for resolving duplicate operator emails during login."""

from datetime import UTC, datetime
from types import SimpleNamespace

from app.api.auth_routes import _choose_best_operator_candidate, _choose_default_workspace_bot


class TestOperatorLoginSelection:
    def test_prefers_workspace_with_bots(self):
        ...
        selected = _choose_best_operator_candidate(
    ...
    def test_falls_back_to_newer_record_when_workspace_counts_match(self):
        older_operator = SimpleNamespace(id=11, client_id=11, created_at=datetime(2026, 1, 10, tzinfo=UTC))
        newer_operator = SimpleNamespace(id=12, client_id=12, created_at=datetime(2026, 3, 10, tzinfo=UTC))

        selected = _choose_best_operator_candidate(
            [older_operator, newer_operator],
            workspace_stats={
                11: {
                    "bot_count": 1,
                    "operator_count": 2,
        ...
        assert selected is newer_operator
