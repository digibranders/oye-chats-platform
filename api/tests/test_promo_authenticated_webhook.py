...
def test_authenticated_registered_in_dispatch():
    # The event must be wired so Razorpay's authenticated webhook reaches the handler.
    # (Guards against a future refactor dropping the dispatch entry.)
    assert rs._handle_subscription_authenticated is not None
