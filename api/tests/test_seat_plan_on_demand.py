...
from app.db.models import AddonPlanCache
...
def test_the_domestic_rail_is_minted_at_base_plus_gst(db, mint):
    plan_id = rs.resolve_addon_plan_id(
        db, kind="seat", item_name="OyeChats operator seat", currency="INR", base_minor=49900, rate_bps=1800
    )
    ...
    row = db.query(AddonPlanCache).one()
    assert row.amount_minor == 58882
    assert row.currency == "INR"


def test_the_export_rail_carries_no_indian_gst(db, mint):
    rs.resolve_addon_plan_id(
        db, kind="seat", item_name="OyeChats operator seat", currency="USD", base_minor=500, rate_bps=1800
    )

    row = db.query(AddonPlanCache).filter_by(currency="USD").one()
    assert row.amount_minor == 500  # $5, not $5.90


def test_the_same_price_is_minted_once_and_reused(db, mint):
    first = rs.resolve_addon_plan_id(
        db, kind="seat", item_name="OyeChats operator seat", currency="INR", base_minor=49900, rate_bps=1800
    )
    second = rs.resolve_addon_plan_id(
        db, kind="seat", item_name="OyeChats operator seat", currency="INR", base_minor=49900, rate_bps=1800
    )

    assert first == second
    assert len(mint) == 1, "a cache hit must not mint a second plan"
    assert db.query(AddonPlanCache).count() == 1


def test_a_new_price_mints_its_own_plan(db, mint):
    old = rs.resolve_addon_plan_id(
        db, kind="seat", item_name="OyeChats operator seat", currency="INR", base_minor=44900, rate_bps=1800
    )
    new = rs.resolve_addon_plan_id(
        db, kind="seat", item_name="OyeChats operator seat", currency="INR", base_minor=49900, rate_bps=1800
    )
    ...
    assert old != new
    assert len(mint) == 2
    assert {r.amount_minor for r in db.query(AddonPlanCache)} == {52982, 58882}
...
def test_a_gst_change_mints_a_fresh_plan_even_though_the_base_is_unmoved(db, mint):
    ...
    at_18 = rs.resolve_addon_plan_id(
        db, kind="seat", item_name="OyeChats operator seat", currency="INR", base_minor=49900, rate_bps=1800
    )
    at_12 = rs.resolve_addon_plan_id(
        db, kind="seat", item_name="OyeChats operator seat", currency="INR", base_minor=49900, rate_bps=1200
    )
...
def test_a_discount_comes_off_the_base_before_tax(db, mint):
    ...
    rs.resolve_addon_plan_id(
        db, kind="seat", item_name="OyeChats operator seat", currency="INR", base_minor=39920, rate_bps=1800
    )
...
def test_the_two_rails_never_share_a_row(db, mint):
    ...
    inr = rs.resolve_addon_plan_id(
        db, kind="seat", item_name="OyeChats operator seat", currency="INR", base_minor=500, rate_bps=0
    )
    usd = rs.resolve_addon_plan_id(
        db, kind="seat", item_name="OyeChats operator seat", currency="USD", base_minor=500, rate_bps=0
    )

    assert inr != usd
    assert db.query(AddonPlanCache).count() == 2
...
def test_a_zero_price_is_refused_rather_than_minted(db, mint):
    ...
    with pytest.raises(ValueError):
        rs.resolve_addon_plan_id(
            db, kind="seat", item_name="OyeChats operator seat", currency="INR", base_minor=0, rate_bps=1800
        )
    assert mint == []


# ── the branding add-on, which had no plan id in any environment ─────────────


def test_branding_removal_is_billable_without_any_configured_plan_id(db, mint):
    """The add-on was priced, advertised, and impossible to buy.

    Every plan seeds `features.branding_removable = false` because removing the
    badge is a standalone purchase, not a tier inclusion. But the purchase
    resolved its plan from `RAZORPAY_BRANDING_PLAN_ID`, which was never set in
    any environment — so the only route to the entitlement raised "not
    configured for the INR rail". Nobody could remove the branding, including
    ...
    assert plan_id == "plan_minted_1"
    row = db.query(AddonPlanCache).filter_by(addon_kind="branding").one()
    assert row.amount_minor == 58882  # ₹499 + 18% GST


def test_a_seat_and_a_branding_removal_at_one_price_get_their_own_plans(db, mint):
    """Same amount, different plans, because the customer reads the name.

    ₹499 buys either add-on today. One Razorpay plan could technically bill
    both, but its item name is what appears on the checkout sheet and the
    invoice — sharing would tell someone buying branding removal that they were
    paying for an operator seat.
    """
    seat = rs.resolve_addon_plan_id(
        db,
        kind="seat",
    ...
    branding = rs.resolve_addon_plan_id(
        db,
        kind="branding",
        item_name="OyeChats branding removal",
        currency="INR",
    ...
    assert seat != branding
    assert db.query(AddonPlanCache).count() == 2
    assert {c["name"] for c in mint} == {
        "OyeChats operator seat INR",
        "OyeChats branding removal INR",
    }


def test_the_branding_add_on_is_reused_on_the_second_purchase(db, mint):
    first = rs.resolve_addon_plan_id(
        db,
        kind="branding",
        item_name="OyeChats branding removal",
