...
# Quotation flow
# ─────────────────────────────────────────────────────────────────────────────
# Default delay (seconds) before the visitor's "Your quotation" document email
# (the one carrying the itemised pricing inline in the body) is sent. The
# visitor's "Your quote request" acknowledgement and the owner notification
# both fire immediately at accept; only the priced document is held back. This
# is now only the DEFAULT: each bot's own quotation_catalog.document_delay_seconds
# (admin-configurable in the dashboard) is what actually governs the delay —
# see quotation_routes._schedule_quotation_emails — and this constant supplies
# that field's default for any bot that has never touched the setting. Set to
# 0 to send immediately by default.
...
# The gate model is DB-backed (pricing_config `model.gate`), not an env var.
# RELEVANCE_THRESHOLD=0.3. Chunks scoring below this cause the gate to fire and block generation
