...
#: The reason line of the team email for an urgent incident. The handoff-request
#: template shows it next to the visitor's contact details.
URGENT_INCIDENT_EMAIL_REASON = "URGENT: a visitor reported an active incident in chat"


async def task_send_urgent_incident_email(ctx: dict, bot_id: int, session_id: str) -> bool:
    """Email the team that a visitor reported an active incident in chat.

    Enqueued by the chat pipeline's urgent route once per conversation, next to
    the urgent inbox notification it writes in the turn. The provider call is
    blocking, so it runs here rather than inside the visitor's streamed reply.

    Unlike ``task_send_visitor_message_email`` this does not require a waiting
    handoff: the visitor may not have filled in the form yet, and the alert is
    the point. Recipients come from the bot's ``handoff_request`` list (falling
    back to ``default``). One recipient failing does not stop the next.
    Returns True when at least one email was handed to the provider.
    """
    import asyncio

    from sqlalchemy import select

    from app.db.models import Bot, LeadInfo
    from app.db.session import SessionLocal
    from app.services.email_service import (
        get_notification_recipients,
        send_handoff_request_email,
    ...
    def _run() -> bool:
        if SessionLocal is None:
            return False
        # Read everything first and release the connection before the provider calls.
        with SessionLocal() as db:
            bot = db.execute(select(Bot).where(Bot.id == bot_id)).scalar_one_or_none()
            if bot is None or not getattr(bot, "email_on_handoff", True):
                return False
            ...
            if not recipients:
                return False
            lead = db.execute(
                select(LeadInfo).where(LeadInfo.session_id == session_id, LeadInfo.bot_id == bot_id)
            ).scalar_one_or_none()
