...
- The business points OyeChats at its website URL (or uploads PDF/DOCX/TXT documents). A crawl pipeline fetches pages HTTP-only, with no local browser, through two interchangeable managed providers: **Jina Reader is the primary and Spider.cloud the fallback** — `CRAWL_PROVIDER_PRIMARY` defaults to `"jina"` (`api/app/config.py:674`). Older material (including root `CLAUDE.md`'s prose) has the two the other way round; the config default is the authority. [T1 — `api/app/config.py:674`, `crawl_orchestrator.py`, `spider_service.py`, `jina_service.py`]
...
- A **relevance gate** (CRAG-style, on by default) has an LLM judge score every retrieved chunk against the question on a 0–1 scale. If every chunk scores below threshold (default 0.3, tunable per bot), the pipeline stops before generation and returns a "can't help" response — it deliberately never lets an ungrounded question reach the answer-writing model. [T1 — `relevance_gate.py` docstring: "If ALL chunks score below the threshold, the gate fires... returns a 'can't help' response without generating an answer from irrelevant context"]
