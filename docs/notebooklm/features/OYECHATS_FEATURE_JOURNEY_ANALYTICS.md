...
- The Journey analytics *view* is gated to the `standard` and `professional` plan slugs (`JOURNEY_ANALYTICS_SLUGS`). Below that, the frontend shows a full-page "locked feature" upgrade card instead of the charts.
...
Confirmed frontend surface: a "Journey" page under the admin's Analytics area (`app/src/features/analytics/JourneyPage.tsx`), scoped to one selected AI Agent (bot), composed of three stacked/paired sections [T1]:
- **`UserJourneyFlow`** — a Sankey-inspired flow diagram: source pages on the left (what visitors viewed before opening chat, heuristically bucketed into a small fixed set of labeled slots — e.g. Home, Pricing, Blog, Product, FAQ, Resources, Other Pages — so the layout stays visually stable even as a business's actual URLs change), a central "Opened Chatbot" node, and outcome destinations on the right. Curve thickness scales with visitor volume [T1, component docstring].
- **`PageInfluence`** — a ranked panel, presumably surfacing the `top_pages` aggregation described above (component present in the same directory; not independently read line-by-line in this pass — see [VERIFY] below).
- **`JourneyOutcomes`** — a panel surfacing conversion/outcome counts (present in the same directory; not independently read line-by-line — see [VERIFY] below).
- **`LeadJourneyFunnel`** — an additional component present in `app/src/features/analytics/` whose exact placement in the page (or whether it renders elsewhere, e.g. on an individual lead's detail view) was not confirmed in this pass [VERIFY].

> **Note on IA drift:** the root master-knowledge doc flags that older technical-story material describes a top-level "Journey" nav item, while the current Admin 2.0 rebuild mandate (`app/CLAUDE.md`) folds Journey into an agent's **Analytics** tab instead [T2, `OYECHATS_MASTER_KNOWLEDGE.md` §11.2]. The `JourneyPage.tsx` component confirmed above is real and current, but exactly which nav path leads to it in the shipped build at production time should be reverified before a script narrates specific on-screen navigation.
...
- Plan gating (`JOURNEY_ANALYTICS_SLUGS = {"standard", "professional"}`) confirmed [T1] in `api/app/services/plan_entitlements_service.py`; frontend enforcement confirmed [T1] in `app/src/features/analytics/JourneyPage.tsx`.
- **[VERIFY]** `PageInfluence.tsx` and `JourneyOutcomes.tsx` were located but not read line-by-line in this pass — their exact displayed metrics are inferred from their filenames and the backend aggregation functions they most plausibly consume (`top_pages` and `summary_counts`/`paths_to_conversion` respectively). Confirm their actual rendered content before scripting specific on-screen numbers or labels.
- **[VERIFY]** `LeadJourneyFunnel.tsx` exists in the same directory but its placement/usage was not traced in this pass — confirm whether it's part of the Journey page, a lead-detail view, or unused/in-progress before depicting it.
