...
**What does it do?** It reads a business's website and uploaded documents, turns that content into a searchable private knowledge base, answers visitor questions from that knowledge base only (not the open internet, not invented content), scores how sales-ready each visitor is, and — when a visitor is ready for a human — hands the conversation to a real team member live — announcing it to the eligible team, holding the visitor in a durable queue, and notifying every device each operator owns until someone claims it. **[T2 — `docs/oyechats-technical-story.md`, Part 0, Part 5, Part 6, Part 7, Part 9; assignment mechanics [T1] — `api/app/services/live_chat_service.py:885`]**
...
**→ Human interaction** — if the visitor asks for a person (or the bot's own answer implies one is needed), the conversation moves through an audited state machine (`bot → waiting → live/closed`), the waiting visitor is announced to every eligible operator at once, and the first operator to accept takes the chat under a race-safe lock. No operator is selected for them: assignment is operator-pull, not automatic routing. **[T1 — `api/app/services/live_chat_service.py:885` `request_handoff`; root `CLAUDE.md` `Operator`, `Department`, `ChatAuditLog` models; `docs/oyechats-technical-story.md` Part 9]**
...
- **Evidence:** **[T1/T2]** root `CLAUDE.md` `BANTSignal` model; the bot's chosen framework lives inside `Bot.bant_config` (JSONB, `models.py:361`), read by `qualification_service._framework_name` — there is no `Bot.qualification_framework` column, and the column of that name is on `ChatSession`, stamping which framework scored a given conversation (`models.py:985`); `docs/oyechats-technical-story.md` Part 7.
...
- **What:** An audited four-state conversation machine (`bot`/`waiting`/`live`/`closed`), a durable queue, and multi-channel operator notification (dashboard, browser push, mobile push, email) with cross-device claim deduplication. Assignment is **broadcast-then-first-accept**: the whole eligible operator pool is notified and whoever claims first gets the chat.
...
- **Caveat — do not say "routed":** a routing module with three strategies (least-busy / round-robin / first-available) exists and `Bot.live_chat_routing_strategy` is a real settable column, but its only entry point `select_operator` (`api/app/services/live_chat_routing_service.py:85`) has **no caller anywhere in the API**, so the setting changes nothing. Never claim OyeChats picks an operator, balances load, or sends a chat to "whoever's free." See `features/OYECHATS_FEATURE_LIVE_CHAT.md` §3.4. **[T1]**
...
- **Caveat:** **[T1 — updated]** IP-based company/organization identification is live, not flag-gated. The lookup runs for every visitor (`api/app/services/ip_intel_service.py`, "Always-on, best-effort signal"), and *display* is plan-gated to `trial`, `professional` and `enterprise` (`VISITOR_INTELLIGENCE_SLUGS`, `plan_entitlements_service.py:531`). What it is **not** is dependable: an IP resolves to an employer only when the visiting company owns its own address block, and the code's own measured result was **0 usable company names out of 10 real production lookups** before filtering was tightened. Present it as an occasional bonus signal on paid plans, never as an always-on feature or a guaranteed per-conversation field. The earlier "metered and currently held behind a flag" framing (`docs/oyechats-technical-story.md` Part 8.2) is superseded.
...
- **IA conflict — RESOLVED against the shipped source; see `OYECHATS_SOURCE_OF_TRUTH.md` for the full log.** Two documents disagreed (`docs/oyechats-technical-story.md` Part 2 and the `app/CLAUDE.md` build mandate), and **the shipped navigation matches neither**. `app/src/shell/nav.ts` is the single definition the rail, breadcrumbs and command palette all read, and it ships: **Home · Inbox · Leads · Journey · Analytics · Chatbots**, with **Billing** and **Settings** in the rail footer. Journey is a top-level route (`/journey`), not an Analytics tab, and the customer-facing noun is **"Chatbots," not "AI Agents."** A chatbot's own tabs are **Overview · Knowledge · Experience · Deploy · Qualification · Quotation · Behaviour** (`AGENT_NAV`, same file) — "Deploy" replaced "Channels" and "Behaviour" replaced "Advanced", per that file's own comments. **[T1 — `app/src/shell/nav.ts:74-110`]** The mandate's six-item list is forward intent, not a description of the build; do not cite it as current fact.
...
- **Relevance:** Medium-high — covers chatbot setup, leads list, live inbox. On-screen labels are now sourced from `app/src/shell/nav.ts` rather than from either narrative document.
