...
Confirmed in `widget/src/app-entry.jsx`: the widget calls `container.attachShadow({ mode: 'open' })` and renders its entire React tree, plus its own `<link>` stylesheet, inside that shadow root under `#oyechats-shadow-inner`. **[T1]** This is what makes the widget collision-proof against arbitrary host-page CSS/JS, and is also why the "Powered by OyeChats" badge is invisible to search crawlers: the widget only mounts after a visitor clicks the launcher, so an in-widget badge is invisible to non-JS-executing crawlers, and the shadow root additionally hides it from any crawler that does run JS but doesn't pierce shadow DOM. A 2026-08 revision added a crawlable `<a>` beside the script tag to work around this; it was withdrawn on 2026-09-07 because it wrote our markup into customer pages. **[T1/T2]**
