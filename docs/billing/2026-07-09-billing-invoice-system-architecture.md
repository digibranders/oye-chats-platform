...
- **Cancel is intent; the gateway cancel is deferred.** `POST /subscriptions/cancel` sets `cancel_at_period_end` and touches nothing at Razorpay. `task_execute_pending_cancellations` (00:03 daily) issues the real `cancel_at_cycle_end=1` once `current_period_end` is within `GATEWAY_CANCEL_LEAD_DAYS` (2), and stamps `gateway_cancel_executed_at`. Cancelling inside that window executes inline. Because Razorpay has no un-cancel, doing this on click destroyed the mandate ~30 days early and made reactivating cost a whole new cycle.
- **Resume has two modes.** Mandate still live (`gateway_cancel_executed_at IS NULL`, confirmed by a `subscription.fetch`) → clear the flag, `mandate_action: "none"`, no checkout, no payment. Mandate already dead → mint a fresh subscription tagged with `prev_razorpay_subscription_id` and return `reauthorise_required`. The re-auth mint passes **`start_at = current_period_end`** so the replacement first charges when the paid period runs out; its activation carries the paid-through period onto the new row and grants no credits, so the existing allowance keeps running. A per-bot row additionally carries `notes.oyechats_bot_id` so the sibling sweep retires the right subscription.
- **Charged backstop.** If the sweep is late and Razorpay renews a cancel-pending subscription anyway, `subscription.charged` cancels it immediately, withholds the credit grant and logs for refund — bounding the worst case at one cycle.
