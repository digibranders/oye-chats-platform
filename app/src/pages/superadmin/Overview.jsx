...
export default function SuperadminOverview() {
    ...
    return (
        <div className="space-y-8 animate-slide-up">
            <div>
                <h1 className="text-2xl font-bold text-secondary-900">Global Overview</h1>
                <p className="text-secondary-500 mt-1">Monitor total platform usage and active clients.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {/* Total Clients Card */}
                <div className="bg-white p-6 rounded-2xl border border-secondary-200 shadow-sm relative overflow-hidden transition-all hover:shadow-md group">
                    ...
                    {isLoading ? (
                        <Loader2 className="animate-spin text-secondary-300 mt-4" size={24} />
                    ) : (
                        <div className="mt-4 flex items-baseline gap-2">
                            <p className="text-3xl font-bold text-secondary-900">{stats?.total_clients || 0}</p>
                ...
                <div className="bg-white p-6 rounded-2xl border border-secondary-200 shadow-sm relative overflow-hidden transition-all hover:shadow-md group">
                    ...
                    {isLoading ? (
                        ...
                        <div className="mt-4">
                            <p className="text-3xl font-bold text-secondary-900">{stats?.total_messages?.toLocaleString() || 0}</p>
                        </div>
                    )}
                </div>
                ...
                <div className="bg-white p-6 rounded-2xl border border-secondary-200 shadow-sm relative overflow-hidden transition-all hover:shadow-md group">
                    ...
                    {isLoading ? (
                        ...
                        <p className="text-3xl font-bold text-secondary-900 mt-4">{stats?.total_sessions?.toLocaleString() || 0}</p>
                    )}
                </div>

            </div>

             <div className="mt-8 p-6 bg-indigo-50 rounded-2xl border border-indigo-100 flex flex-col items-center justify-center text-center">
                 <Bot size={40} className="text-indigo-400 mb-4" />
                 <h2 className="text-lg font-bold text-secondary-900 mb-2">Platform Health Optimal</h2>
