...
const STATES: readonly { value: PreviewState; labelKey: string; label: string }[] = [
  { value: 'welcome', labelKey: 'agents.previewStateChat', label: 'Chat' },
  { value: 'waiting', labelKey: 'agents.previewStateWaiting', label: 'Waiting' },
  { value: 'offline', labelKey: 'agents.previewStateOffline', label: 'Offline' },
];
  ...
  return (
    ...
    <Card>
      ...
      <CardBody className="flex flex-col gap-3">
        <SegmentedControl
          items={STATES.map((o) => ({ ...o, label: t(o.labelKey) || o.label }))}
          value={state}
          onChange={setState}
        ...
        {answerStale ? (
          <Alert tone="warning" title={t('agents.repliesStillUseYourSaved') || 'Replies still use your saved settings'}>
            {t('agents.coloursAndWordingUpdateAsYouType') ||
              'Colours and wording above update as you type, but an answer is generated by the chatbot as it is saved.'}
          </Alert>
        ) : null}
      </CardBody>
