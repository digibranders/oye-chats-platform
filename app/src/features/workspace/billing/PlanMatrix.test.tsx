...
describe('PlanMatrix — AI agents included row', () => {
  it('renders the unlimited sentinel as "Unlimited", never as a count of -1', () => {
    renderMatrix();
    expect(rowValues('AI agents included').at(-1)).toBe('Unlimited');
  });
  ...
  it('renders a single included agent as "1" on every capped tier', () => {
    ...
    expect(rowValues('AI agents included')).toEqual(['1', '1', '1', '1', 'Unlimited']);
  });
  ...
  it('reads the same string quota the backend does ("-1" from JSONB counts)', () => {
    ...
    expect(rowValues('AI agents included')).toEqual(['Unlimited']);
  });
  ...
  it('claims nothing for a plan row that declares no agent quota', () => {
    ...
    expect(rowValues('AI agents included')).toEqual(['-']);
  });
  ...
  it('leads the Usage group, above credits', () => {
    ...
    expect(labels.indexOf('AI agents included')).toBe(0);
    expect(labels.indexOf('AI agents included')).toBeLessThan(labels.indexOf('Credits / month'));
  });
});
