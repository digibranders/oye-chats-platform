    ...
    <span className="flex flex-wrap items-center gap-1">
      {pages.map((page, index) => (
        <span key={`${page}-${index}`} className="flex items-center gap-1">
          ...
          <span className="figure break-all text-xs text-text-primary">{page}</span>
        </span>
      ))}
    </span>
  );
...
export function JourneyFlow({
  ...
  return (
    <Card>
      ...
      />
      <CardBody className="grid gap-6 lg:grid-cols-[1.4fr_0.8fr_1fr]">
        {/* Before the chat */}
        <section aria-labelledby="journey-before">
          <h3 id="journey-before" className="text-sm font-semibold text-text-primary">
            Before the chat
          </h3>
          <p aria-live="polite" className="mt-1 text-xs text-text-secondary">
            {selectedOutcome
              ? `${formatNumber(rows.length)} ${rows.length === 1 ? 'route' : 'routes'} attributed to ${outcomeLabelFor(selectedOutcome).toLowerCase()}`
              : `${formatNumber(sequences.sessions_with_pre_chat)} of ${formatNumber(sequences.total_sessions)} conversations were browsing first`}
          </p>
        ...
        <section aria-labelledby="journey-chat">
          <h3 id="journey-chat" className="text-sm font-semibold text-text-primary">
            The chat
          </h3>
          <div className="mt-3 rounded-md border border-border bg-surface-sunken px-4 py-4 text-center">
            <MessageSquare aria-hidden className="mx-auto h-4 w-4 text-text-tertiary" />
            <p className="figure mt-2 text-xl font-semibold text-text-primary">
              {formatNumber(summary.sessions_with_journey)}
            </p>
            <p className="text-xs text-text-secondary">
              {summary.sessions_with_journey === 1 ? 'tracked journey' : 'tracked journeys'}
            </p>
            <p className="mt-3 border-t border-border pt-3 text-xs text-text-secondary">
              <span className="figure text-text-primary">
                {formatNumber(summary.leads_captured)}
              </span>{' '}
              left their details
            </p>
          </div>
        ...
        <section aria-labelledby="journey-after">
          <h3 id="journey-after" className="text-sm font-semibold text-text-primary">
            After the chat
          </h3>
          ...
          <h4 className="mt-5 text-xs font-semibold text-text-primary">Where they went next</h4>
