...
### AR-04 update (implementation pass) — already fixed pre-review; comments were stale, not the code

Follow-up `git log -S` on `api/app/main.py` found commit `393a15d` ("fix(observability): remove broken LiteLLM langfuse callback; add OTEL 1s flush", **2026-06-29**, ~9 days before this review) had already removed the exact broken code path: LiteLLM's built-in `"langfuse"` success-callback registration (the one that calls the v2/v3-only `langfuse.version.__version__`). The Jun-26 `AttributeError` spam captured in the live-verification section above was from a **process instance that predates this fix** (`PID 1103`), not the currently-deployed code — consistent with the zero recurrences already observed after the `13:07:21` restart. Checked upstream: litellm 1.91.0 (newest release at the time of this check) still contains the same broken `langfuse.version.__version__` reference in its *built-in* callback — so downgrading langfuse to the 3.x line to match it, which is what a naive reading of this finding would suggest, **would have been the wrong fix**: `app/core/langfuse_client.py` is deliberately written against the v4-only API (`get_client()`, `start_as_current_observation`), which doesn't exist in langfuse 3.x. The only remaining work was two now-stale comments in `main.py`/`config.py` left over from before the Jun-29 fix, describing a callback registration that no longer exists — these have been corrected, and three regression tests added (`tests/test_langfuse_litellm_compat.py`) pinning: (1) the broken callback is never re-registered, (2) the pinned langfuse package still exposes the v4 `get_client` API, and (3) `langfuse_generation()` runs end-to-end against the real (unmocked) langfuse + litellm packages without raising. AR-04 required no functional code change — just documentation cleanup and regression coverage for an already-shipped fix.

### AR-08 update (implementation pass) — descoped; needs a dedicated refactor project, not a batch fix

Investigated fully before writing any code. The `with get_session() as session:` block at `rag_service.py:3895` (current line numbers as of this pass) does not end after the initial bot-lookup/message-persist — it spans **~739 lines**, through the `finally:` at `~4634`, i.e. the entire body of `rag_pipeline_stream`. `session` and the ORM `bot` object it loads are both referenced continuously throughout that span, including **after** the LLM generation call completes: `_resolve_meeting_booking(bot, session, session_id, bot_id)` (`rag_service.py:514`) runs live queries against the same session near the very end of the function, and roughly a dozen other `getattr(bot, ...)` reads and `session.commit()` calls (cache-hit early returns, refusal paths, the final-answer persist) are interleaved with the async LLM/retrieval work in between.

This rules out a partial fix: wrapping only the opening bot-lookup-and-commit segment in `asyncio.to_thread` (mirroring the CAG-lite helper pattern the same function already uses correctly elsewhere) would not reduce event-loop blocking, because `session` continues to be used synchronously on the event-loop thread for the rest of the function regardless. A real fix requires converting the **entire** function from "one long-lived synchronous session for the whole streaming request" to "many short-lived sessions, each opened/used/closed via `asyncio.to_thread`, passing plain data — not lazy-loading-dependent ORM objects — between segments." That is a substantial, high-blast-radius rewrite of the single highest-traffic function in the platform (every chat message streams through it), touching every early-return branch (greeting/refusal/cache-hit/pivot), the meeting-booking resolution path, and the final-answer commit — each of which would need its own careful session-boundary redesign and dedicated integration testing, ideally with staging traffic shadowing, not a rushed change squeezed into a 46-item sequential batch.

**Decision**: left uncoded. Recommend scoping this as its own dedicated project (a design note + incremental PRs, not a single diff), separate from this review's implementation pass. The original "M" effort estimate in Section 2 undersells this — closer to "L" once the actual session-lifetime span was measured.
